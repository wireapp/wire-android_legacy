...
class UserLocalDataSourceTest : UnitTest() {
    ...
    fun `Given changeAccentColor is called, when dao result is successful, then return the data`() {
        runBlockingTest {
            usersLocalDataSource.changeAccentColor(TEST_COLOR)

            verify(userDbService).updateAccentColor(eq(TEST_USER_ID), eq(TEST_COLOR))

            usersLocalDataSource.changeAccentColor(TEST_COLOR).isRight shouldBe true
        }
    }
    ...
    fun `Given changeAccentColor is called, when dao result is cancelled, then return error`() {
        runBlockingTest {
            ...
            usersLocalDataSource.changeAccentColor(TEST_COLOR).isRight shouldBe false
        }
    }
