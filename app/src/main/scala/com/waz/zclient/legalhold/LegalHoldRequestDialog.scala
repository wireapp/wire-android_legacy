...
class LegalHoldRequestDialog extends ConfirmationWithPasswordDialog {
  ...
  override lazy val isPasswordManagedBySSO: Boolean = getArguments.getBoolean(ARG_IS_PASSWORD_SSO_MANAGED)
  ...
  override lazy val message: String = {
    val stringRes =
      if (isPasswordManagedBySSO) R.string.legal_hold_request_dialog_message_for_sso
...
object LegalHoldRequestDialog {
  ...
  private val ARG_IS_PASSWORD_SSO_MANAGED = "LegalHold_arg_isPasswordSSOManaged"
  private val ARG_CLIENT_FINGERPRINT = "LegalHold_arg_fingerprint"
  private val ARG_SHOW_ERROR = "LegalHold_arg_showError"

  def newInstance(isPasswordManagedBySSO: Boolean, fingerprint: String, showError: Boolean) : LegalHoldRequestDialog =
    returning(new LegalHoldRequestDialog) {
      _.setArguments(returning(new Bundle()) { args =>
        ...
        args.putBoolean(ARG_IS_PASSWORD_SSO_MANAGED, isPasswordManagedBySSO)
