...
object MsgPart {
  ...
  def apply(msgType: Message.Type, isOneToOne: Boolean): MsgPart = {
    ...
    msgType match {
      ...
      case OTR_ERROR | OTR_ERROR_FIXED | OTR_DEVICE_ADDED | OTR_IDENTITY_CHANGED | OTR_UNVERIFIED | OTR_VERIFIED |
           HISTORY_LOST | STARTED_USING_DEVICE | OTR_MEMBER_ADDED | RESTRICTED_FILE | SESSION_RESET => OtrMessage
      case KNOCK => Ping
      case RENAME => Rename
