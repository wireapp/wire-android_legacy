...
class SoundControllerImpl(implicit inj: Injector, cxt: Context)
  ...
  private val mediaManager   = zms.flatMap(z => Signal.fromFuture(z.mediamanager.mediaManager))
  ...
  override def isVibrationEnabled(userId: UserId): Boolean = {
    (for {
      zms <- Signal.fromFuture(accountsService.getZms(userId)).collect { case Some(v) => v }
