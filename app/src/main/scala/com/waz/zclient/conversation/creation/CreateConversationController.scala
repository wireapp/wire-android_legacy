...
class CreateConversationController(implicit inj: Injector, ev: EventContext)
  ...
  def createConversation(): Future[ConvId] =
    for {
      ...
      _ = verbose(l"ROL creating conv with  ${userIds.size} users, ${integrationIds.size} bots, shouldFullConv $shouldFullConv, teamOnly $teamOnly and readReceipts $readReceipts")
