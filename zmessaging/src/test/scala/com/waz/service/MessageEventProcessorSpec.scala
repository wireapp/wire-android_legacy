...
class MessageEventProcessorSpec extends AndroidFreeSpec with Inside with DerivedLogTag {
  ...
  def getProcessor = {
    ...
    new MessageEventProcessor(selfUserId, storage, content, assets, replyHashing, msgsService, convsService, convs, downloadStorage, ZMessaging.currentGlobal, prefs, otrClientsStorage)
  }
