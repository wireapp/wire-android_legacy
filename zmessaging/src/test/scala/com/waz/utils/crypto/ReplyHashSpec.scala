...
class ReplyHashSpec extends AndroidFreeSpec {
  ...
  feature("hashing of quoted messages") {
    ...
    scenario("asset id 1") {
      ...
      getReplyHashing.hashAsset(assetId, timestamp1).bytes.hex shouldEqual "bf20de149847ae999775b3cc88e5ff0c0382e9fa67b9d382b1702920b8afa1de"
    }

    scenario("asset id 2") {
      val assetId = RAssetId.apply("3-3-3-82a62735")
      getReplyHashing.hashAsset(assetId, timestamp2).bytes.hex shouldEqual "2235f5b6c00d9b0917675399d0314c8401f0525457b00aa54a38998ab93b90d6"
    }

    scenario("emojis") {
      val str = "Hello \uD83D\uDC69\u200D\uD83D\uDCBB\uD83D\uDC68\u200D\uD83D\uDC69\u200D\uD83D\uDC67!"
      val hash = getReplyHashing.hashTextReply(str, timestamp1).bytes.hex
...
/*    scenario("arabic") {
    ...
    scenario("check location 1") {
      ...
      getReplyHashing.hashLocation(lat, lng, timestamp1).bytes.hex shouldEqual "56a5fa30081bc16688574fdfbbe96c2eee004d1fb37dc714eec6efb340192816"
    }

    scenario("check location 2") {
      val lat: Float = 51.509143.toFloat
      val lng: Float = -0.117277.toFloat
      getReplyHashing.hashLocation(lat, lng, timestamp1).bytes.hex shouldEqual "803b2698104f58772dbd715ec6ee5853d835df98a4736742b2a676b2217c9499"
    }

    /* Convertion of Array[Byte] to a hexidecimal string without relying on Byte64.decode
