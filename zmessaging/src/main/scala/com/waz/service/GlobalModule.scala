...
class GlobalModuleImpl(val context: AContext, val backend: BackendConfig) extends GlobalModule { global =>
  ...
  lazy val globalLoader:        AssetLoader                      = new AssetLoaderImpl(context, network, globalAssetClient, audioTranscoder, videoTranscoder, cache, imageCache, bitmapDecoder, trackingService)(backend, requestInterceptor)
  //end of warning...
