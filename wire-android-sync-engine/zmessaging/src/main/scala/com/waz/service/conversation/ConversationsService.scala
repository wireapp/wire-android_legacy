...
class ConversationsServiceImpl(teamId:          Option[TeamId],
  ...
  private def processUpdateEvent(conv: ConversationData, ev: ConversationEvent) = ev match {
    ...
    case MemberJoinEvent(_, _, _, ids, us, _) =>
      ...
      for {
        ...
        _          <- membersStorage.add(conv.id, membersWithRoles)
    ...
    case MemberUpdateEvent(_, _, _, state, _) => content.updateConversationState(conv.id, state)
  ...
  private def deleteTempConversation(convId: ConvId) = for {
    ...
    _ <- rolesStorage.removeByConvId(convId)
  } yield ()
  ...
  private def deleteConversation(convData: ConversationData): Future[Unit] = (for {
      ...
      _              <- rolesStorage.removeByConvId(convData.id)
    } yield ()).recoverWith {
      case ex: Exception =>
  ...
  override def setConversationRole(convId: ConvId, userId: UserId, role: ConversationRole): Future[Unit] =
    for {
      _ <- membersStorage.update(convId, userId, role)
      _ <- sync.updateConversationRole(convId, userId, role)
    } yield ()
}
