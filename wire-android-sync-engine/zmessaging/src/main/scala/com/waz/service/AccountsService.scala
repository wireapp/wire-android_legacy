...
class AccountsServiceImpl(val global: GlobalModule, val backupManager: BackupManager, val kotlinLogoutEnabled: Boolean = false)
  ...
  override lazy val activeAccount = activeAccountManager.flatMap[Option[AccountData]] {
    case Some(am) => Signal.fromFuture(storage).flatMap(_.optSignal(am.userId))
    case None     => Signal.const(None)
  }
  ...
  override lazy val activeZms = activeAccountManager.flatMap[Option[ZMessaging]] {
    case Some(am) => Signal.fromFuture(am.zmessaging.map(Some(_)))
    case None     => Signal.const(None)
  }

  override lazy val zmsInstances = (for {
    ams <- accountManagers
    zs  <- Signal.sequence(ams.map(am => Signal.fromFuture(am.zmessaging)).toSeq: _*)
  } yield
