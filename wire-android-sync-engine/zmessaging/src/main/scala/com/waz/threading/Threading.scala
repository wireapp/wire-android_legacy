...
object Threading {
  ...
  implicit class RichSignal[E](val signal: Signal[E]) extends AnyVal {
    def onUi(subscriber: Subscription.Subscriber[E])(implicit context: EventContext): Subscription =
  ...
  /**
    ...
    */
  val ImageDispatcher: DispatchQueue = DispatchQueue(Cpus - 1, Background, Some("ImageDispatcher"))
