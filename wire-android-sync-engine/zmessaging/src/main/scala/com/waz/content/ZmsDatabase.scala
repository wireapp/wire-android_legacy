...
class ZmsDatabase(user: UserId, context: Context, tracking: TrackingService) extends Database {
  override implicit val dispatcher: DispatchQueue = DispatchQueue(concurrentTasks = 1, executor = Threading.IOThreadPool, name = Some("ZmsDatabase_" + user.str.substring(24)))
