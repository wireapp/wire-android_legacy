...
import com.wire.signals.{CancellableFuture, DispatchQueue, EventStream, SerialDispatchQueue, Signal, SourceStream}
...
class MessagesStorageImpl(context:     Context,
  ...
  private implicit val dispatcher: DispatchQueue = SerialDispatchQueue(name = "MessagesStorage")
  ...
  def unreadCount(conv: ConvId): Signal[Int] = Signal.fromFuture(msgsIndex(conv)).flatMap(_.signals.unreadCount).map(_.messages)

  def lastRead(conv: ConvId) = Signal.fromFuture(msgsIndex(conv)).flatMap(_.signals.lastReadTime)
