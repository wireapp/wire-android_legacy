...
class MessageEventProcessorSpec extends AndroidFreeSpec with Inside with DerivedLogTag {
  ...
  feature("Push events processing") {
    ...
    scenario("Process MemberJoin events sent from other user") {
      ...
      clock.advance(5.seconds)
      val event = MemberJoinEvent(conv.remoteId, RemoteInstant(clock.instant()), sender, membersAdded, membersAdded.map(_ -> ConversationRole.AdminRole.label))
    ...
    scenario("Discard duplicate system message events") {
      ...
      testRound(MemberJoinEvent(conv.remoteId, RemoteInstant(clock.instant()), sender, membersAdded, membersAdded.map(_ -> ConversationRole.AdminRole.label)))
