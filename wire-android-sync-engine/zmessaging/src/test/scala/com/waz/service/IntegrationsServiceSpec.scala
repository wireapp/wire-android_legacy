...
class IntegrationsServiceSpec extends AndroidFreeSpec {
  ...
  scenario("create new conversation with service if none exists") {
    ...
    (convsUi.createGroupConversation _).expects(Option.empty[Name], Map.empty[UserId, ConversationRole], false, 0).returning(Future.successful(createdConv, createConvSyncId))
